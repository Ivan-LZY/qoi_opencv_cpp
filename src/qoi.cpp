...
std::vector<unsigned char> encode(std::vector<unsigned char> const& orig_pixels, std::uint32_t width, std::uint32_t height, unsigned char channels) {
    ...
    for (std::size_t pixel_index = 0; pixel_index < size; pixel_index += head.channels) {
        if (head.channels == 4) {
            pixel = qoi::RGBA{orig_pixels[pixel_index + 2], orig_pixels[pixel_index + 1], orig_pixels[pixel_index + 0], orig_pixels[pixel_index + 3]}; //changed to fit opencv BGR format 
        } else {
            pixel.r = orig_pixels[pixel_index + 2]; //initially 0, changed to fit opencv BGR format 
            pixel.g = orig_pixels[pixel_index + 1];
            pixel.b = orig_pixels[pixel_index + 0]; //inititally 2, changed to fit opencv BGR format 
        }
